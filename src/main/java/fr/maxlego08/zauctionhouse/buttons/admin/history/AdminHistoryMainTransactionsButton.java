...
public class AdminHistoryMainTransactionsButton extends TargetHelper {
    ...
    public @Nullable ItemStack getCustomItemStack(@NotNull Player player, boolean useCache, @NotNull Placeholders placeholders) {
        ...
        getTargetUniqueId(player).ifPresent(uuid -> {
            var list = this.plugin.getStorageManager().with(TransactionRepository.class).selectByPlayer(uuid);

            placeholders.register("transactions", String.valueOf(list.size()));
    ...
    public void onClick(@NotNull Player player, @NotNull InventoryClickEvent event, @NotNull InventoryEngine inventory, int slot, @NotNull Placeholders placeholders) {
        ...
        cache.remove(PlayerCacheKey.ADMIN_TRANSACTIONS_DATA, PlayerCacheKey.ADMIN_TRANSACTIONS_LOADING,
                PlayerCacheKey.ADMIN_TRANSACTIONS_STATUS_FILTER, PlayerCacheKey.ADMIN_TRANSACTIONS_DATE_FILTER);
